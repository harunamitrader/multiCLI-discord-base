...
- `data\state.json`（runtime snapshot / safe-side recovery 用）
- `data\logs\`（Codex / scheduler などの運用ログ）
- `data\memory\`
- `data\memory-automation\`
- `data\schedules\`
...
- assistant の保存本文は、必要最低限の trim を除き **ユーザープロンプト後の raw 寄り transcript** を優先する
...
- Terminal を開いただけでは spawn しない
- 未起動 agent は黒画面のまま表示し、Start ボタン / main chat 送信 / terminal 補助入力送信のいずれかで起動する

### 6.4.1 workspace sidebar

- workspace sidebar は active / inactive の 2 分割ではなく、単一 ordered list で表示する
- 各 workspace accordion 内の agent card には terminal 状態 badge を表示する
- 状態は最低でも `起動前 / 起動中 / 稼働中 / 入力待ち / エラー` を区別する
- `起動前` の agent card には Start ボタンを表示する

### 6.5 settings

- global default workdir
- workspace 名 / workdir / parent agent / Discord channel binding / context injection
- workspace memory markdown
- durability / resume（resume binding、last run state、operation audits）
- Git safety（checkpoint、rollback preview / apply）
- task coordination（claim / release / handoff）
- workspace profile（mode / persona / autonomy / notes）
- review / isolation（diff preview、worktree status、isolated worktree）
- semantic locks（list / claim / release）
- automation / extensions（dashboard、task board、workflow validation、drift、memory automation、skills、MCP）
...
- 再送信時は同じ PTY を使う
- process が未起動なら **Start ボタン / main chat 送信 / terminal 補助入力送信** のいずれかで spawn する
- Terminal 表示や PTY WebSocket attach だけでは spawn しない
...
- 最終的な assistant text は、必要最低限の trim を除き prompt 後の raw 寄り transcript を DB に保存する
...
- `GET /api/workspaces/:id/extensions/overview`
- `GET /api/workspaces/:id/checkpoints`
- `POST /api/workspaces/:id/checkpoints`
- `POST /api/workspaces/:id/rollback/preview`
- `POST /api/workspaces/:id/rollback/apply`
- `GET /api/workspaces/:id/review`
- `GET /api/workspaces/:id/worktree`
- `POST /api/workspaces/:id/worktree`
- `GET /api/workspaces/:id/locks`
- `POST /api/workspaces/:id/locks/claim`
- `POST /api/workspaces/:id/locks/release`
- `GET /api/workspaces/:id/resume-bindings`
- `GET /api/workspaces/:id/audits`
- `GET /api/workspaces/:id/tasks`
- `POST /api/workspaces/:id/tasks/claim`
- `POST /api/workspaces/:id/tasks/release`
- `GET /api/workspaces/:id/handoffs`
- `POST /api/workspaces/:id/handoffs`
- `GET /api/workspaces/:id/terminal-states`
...
- `GET /api/memory/workspaces/:id`
- `PATCH /api/memory/workspaces/:id`
- `GET /api/workspaces/:id/memory/consolidation/preview`
- `POST /api/workspaces/:id/memory/consolidation/apply`
- `GET /api/workspaces/:id/memory/diary/preview`
- `POST /api/workspaces/:id/memory/diary/apply`
- `GET /api/workspaces/:id/memory/dreaming/preview`
- `POST /api/workspaces/:id/memory/dreaming/apply`
- `POST /api/workspaces/:id/skills/sync`
- `POST /api/workspaces/:id/mcp/sync`
...
- `POST /api/agents/:name/prewarm` は手動 Start 用の明示起動 API として使う
- `POST /api/agents/:name/run` は未起動 PTY も必要に応じて起動してから同じ shared PTY に prompt を流す
...
- Workspace profile surface を追加済み
- Review / isolation、semantic lock、memory automation、skills / MCP sync を回帰へ反映済み
- Phase E の dashboard / task board / workflow validation は headless UI smoke まで確認済み
