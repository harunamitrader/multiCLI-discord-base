...
加えて現在は、workspace profile / review / isolation / semantic lock / automation surfaces が UI settings に揃っており、Discord 側の runtime command も同じ workspace / PTY 状態を前提に運用される。
...
- review / worktree / semantic lock / workspace profile を含む workspace runtime と Discord binding の整合
...
現在の主作業は shared PTY 前提の運用 surface を README / 仕様書 / runbook に揃えることであり、Discord 連携自体はその前提の上で安定運用フェーズに入っている。
