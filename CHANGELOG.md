...
- progress tracker が未作成のまま終了したケースでも、Discord に最終状態を必ず投稿するよう改善
- `stop` の Discord 返信に、実行中キャンセルとキュー削除件数を含めるよう改善
- `stop` が未処理キューも停止する挙動をローカルモックと実チャンネルで再確認
- `error` 時に Discord へ `> Error (...)` とエラー本文が届くことを実チャンネルで再確認
- HTML / service worker / manifest / package version を `0.1.66` に更新
